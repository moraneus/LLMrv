...
#    Few-shot examples are auto-generated by the grounding LLM —
#    just provide a description and role.
guard = TemporalGuard(
    propositions=[
    ...
    # Few-shot examples are auto-generated on first run, then cached to
    # .temporalguard/ so subsequent runs don't need to regenerate them.
)
...
A **proposition** is an atomic statement about a single message. TemporalGuard evaluates propositions using LLM-as-judge grounding with few-shot examples.

```python
from temporalguard import Proposition

# Minimal — few-shot examples are auto-generated by the grounding LLM:
prop = Proposition(
    prop_id="p_harmful",
    role="user",
...
# With your own examples (overrides auto-generation for this proposition):
prop = Proposition(
    prop_id="p_harmful",
    role="user",
    description="The user requests instructions for causing physical harm",
    few_shot_positive=[             # Examples that SHOULD match
...
- `few_shot_positive` and `few_shot_negative` are optional — if omitted, the SDK auto-generates 5 of each using the grounding LLM, then caches them to disk. If you provide your own, they are used as-is.
...
For larger policy sets, define everything in YAML. Few-shot examples are optional — omit them and the SDK auto-generates them on first run:

```yaml
# policies.yaml
propositions:
  # Minimal — just id, role, and description. Few-shots are auto-generated.
  - id: p_fraud
    role: user
  ...
  # You can also provide your own few-shot examples to override auto-generation:
  - id: p_violence
    role: user
    description: "The user requests instructions for causing physical harm"
    few_shot_positive:
    ...
    few_shot_negative:
      - "What safety regulations exist for mining explosives?"
      - "How do hospitals treat burn victims?"
...
On the first run, the chat server auto-generates few-shot examples for propositions that don't have them. These are cached in a Docker volume (`temporalguard-cache`), so subsequent `docker compose up` starts instantly without regenerating.
...
Each policy has **user-side propositions** (what the user asks) and **assistant-side propositions** (what the assistant does). Most propositions have no manual few-shot examples — the SDK auto-generates them on first startup using the grounding LLM, then caches them. The `p_hack` proposition has user-provided examples to demonstrate the override capability.
...
To define your own policies, edit `examples/policies.yaml`. Just provide descriptions — few-shot examples are auto-generated:
...
propositions:
  - id: p_my_topic            # Unique ID (used in formulas)
    ...
    # few_shot_positive/negative are OPTIONAL — auto-generated if omitted
...
```bash
docker compose down           # Stop services (keeps model + few-shot cache)
docker compose down -v        # Stop + remove all volumes (re-downloads model, regenerates few-shots)
```
