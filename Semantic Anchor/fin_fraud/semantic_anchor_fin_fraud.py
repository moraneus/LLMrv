#!/usr/bin/env python3
"""
Semantic Anchor Evaluator: fin_fraud
======================================

Auto-generated by semantic_anchor_generator.py

Loads anchors from: anchors_list_fin_fraud.json

Usage:
  python semantic_anchor_fin_fraud.py                       # Interactive (cosine mode)
  python semantic_anchor_fin_fraud.py --mode nli             # NLI entailment scoring
  python semantic_anchor_fin_fraud.py --mode hybrid          # Weighted blend: NLI + cosine KNN (recommended)
  python semantic_anchor_fin_fraud.py --mode llm             # LLM-as-judge scoring
  python semantic_anchor_fin_fraud.py --compare              # Compare all modes side-by-side
  python semantic_anchor_fin_fraud.py --compare -f input.txt # Compare all modes on file
  python semantic_anchor_fin_fraud.py --verbose              # Full table
  python semantic_anchor_fin_fraud.py --show-examples        # View anchors
  python semantic_anchor_fin_fraud.py --file input.txt       # File mode (### separated)
  python semantic_anchor_fin_fraud.py --graph                # 3D anchor spread visualization
...
Scoring modes:
  ...
  llm:     LLM-as-judge. Requires config_fin_fraud.ini with provider, model, api_key.
...
def _load_llm_config(silent=False):
    ...
    _llm_config = {
        ...
        "site_url": config.get("llm_judge", "site_url", fallback=""),
        "site_name": config.get("llm_judge", "site_name", fallback=""),
    }
    return _llm_config
...
def main():
    ...
    if mode in ("nli", "hybrid", "compare"):
        ...
        if prov in ("openrouter", "ollama", "local", "lmstudio", "vllm"):
